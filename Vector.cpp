...
Vector::Vector(int s, int k)
{
	...
	for (int i = 0; i < size; i++)
		data[i] = k;
	beg.elem = &data[0]; end.elem = &data[size];
}
...
Vector& Vector::operator=(const Vector& a)
{
	...
	if (data != 0) delete[]data; data = new int[size];
	for (int i = 0; i < size; i++)
		data[i] = a.data[i]; return *this;
	beg = a.beg; end = a.end; return *this;
}
...
Vector Vector::operator+(const Vector& a)//+k
{
